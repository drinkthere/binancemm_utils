...
let api = function Binance(options = {}) {
    ...
    let papi = "https://papi.binance.com/papi/";
    ...
    let pstream = "wss://fstream.binance.com/pm/stream?streams=";
    let pstreamSingle = "wss://fstream.binance.com/pm/ws/";
    let orderWs = "wss://ws-fapi.binance.com/ws-fapi/v1?returnRateLimits=false";
    ...
    Binance.pmSubscriptions = {};
    Binance.ohlcLatest = {};
    Binance.klineQueue = {};
    Binance.ohlc = {};

    Binance.orderConnections = {};
    ...
    function setOptions(opt = {}, callback = false) {
        ...
        if (typeof Binance.options.test === "undefined")
            Binance.options.test = default_options.test;
        ...
        if (typeof Binance.options.urls !== "undefined") {
            ...
            if (typeof urls.fstreamTest === "string")
                fstreamTest = urls.fstreamTest;
            if (typeof urls.fstreamSingleTest === "string")
                fstreamSingleTest = urls.fstreamSingleTest;
            if (typeof urls.dstream === "string") dstream = urls.dstream;
            if (typeof urls.dstreamSingle === "string")
                dstreamSingle = urls.dstreamSingle;
            if (typeof urls.dstreamTest === "string")
                dstreamTest = urls.dstreamTest;
    ...
    const reqHandler = (cb) => (error, response, body) => {
        ...
        if (response) {
            ...
            if (response.headers) {
                Binance.info.usedWeight =
                    response.headers["x-mbx-used-weight-1m"] || 0;
                Binance.info.orderCount1s =
                    response.headers["x-mbx-order-count-1s"] || 0;
                Binance.info.orderCount1m =
                    response.headers["x-mbx-order-count-1m"] || 0;
                Binance.info.orderCount1h =
                    response.headers["x-mbx-order-count-1h"] || 0;
                Binance.info.orderCount1d =
                    response.headers["x-mbx-order-count-1d"] || 0;
            }
        }
        if (!cb) return;
    ...
    const proxyRequest = (opt, cb) => {
        const req = request(addProxy(opt), reqHandler(cb)).on(
            "error",
            (err) => {
                cb(err, {});
            }
    ...
    const requireApiSecret = function (
        source = "requireApiSecret",
        fatalError = true
    ) {
        if (!Binance.options.APIKEY) {
            if (fatalError) throw Error(`${source}: Invalid API Key!`);
    ...
    const signedRequest = (
        ...
        if (typeof data.recvWindow === "undefined")
            data.recvWindow = Binance.options.recvWindow;
        let query =
    ...
    const order = (
        ...
        if (
            opt.type == "MARKET" &&
            typeof flags.quoteOrderQty !== "undefined"
        ) {
    ...
    const marginOrder = (
        ...
        if (typeof flags.type !== "undefined") opt.type = flags.type;
        if (typeof flags.isIsolated !== "undefined")
            opt.isIsolated = flags.isIsolated;
        if (opt.type.includes("LIMIT")) {
            opt.price = price;
            if (opt.type !== "LIMIT_MAKER") {
                opt.timeInForce = "GTC";
        ...
        if (typeof flags.newClientOrderId !== "undefined")
            opt.newClientOrderId = flags.newClientOrderId;
        if (typeof flags.sideEffectType !== "undefined")
    ...
    const pmUmOrder = async (
        ...
        return promiseRequest("v1/um/order", params, {
            base: papi,
            type: "TRADE",
            method: "POST",
        });
    ...
    const pmCmOrder = async (
        ...
        return promiseRequest("v1/cm/order", params, {
            base: papi,
            type: "TRADE",
            method: "POST",
        });
            ...
            try {
                request(addProxy(opt), (error, response, body) => {
                    ...
                    try {
                        ...
                        return reject(
                            `promiseRequest error #${response.statusCode}`
                        );
                    }
                }).on("error", reject);
            } catch (err) {
    ...
    const handleSocketOpen = function (opened_callback) {
        ...
        if (Object.keys(Binance.subscriptions).length === 0) {
            Binance.socketHeartbeatInterval = setInterval(
                socketHeartbeat,
                30000
            );
        }
    ...
    const handleSocketClose = function (reconnect, code, reason) {
        ...
        if (Binance.options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && parseInt(this.endpoint.length, 10) === 60)
                ...
                Binance.options.log(
                    "WebSocket reconnecting: " + this.endpoint + "..."
                );
            try {
                reconnect();
            } catch (error) {
                Binance.options.log(
                    "WebSocket reconnect error: " + error.message
                );
            }
        }
    };
    ...
    const subscribe = function (
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log("using socks proxy server " + socksproxy);
            ...
            if (Binance.options.verbose)
                Binance.options.log("using proxy server " + agent);
            ws = new WebSocket(stream + endpoint, { agent: agent });
        } else {
    ...
    const subscribeCombined = function (
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log("using proxy server " + httpsproxy);
            let config = url.parse(httpsproxy);
            let agent = new HttpsProxyAgent(config);
            ws = new WebSocket(combineStream + queryParams, { agent: agent });
        } else {
        ...
        ws.on("message", (data) => {
            try {
                ...
                Binance.options.log(
                    "CombinedStream: Parse error: " + error.message
                );
            }
        });
        return ws;
    ...
    const handleFuturesSocketClose = function (reconnect, code, reason) {
        ...
        if (
            Binance.futuresSubscriptions &&
            Object.keys(Binance.futuresSubscriptions).length === 0
        ) {
            clearInterval(Binance.socketHeartbeatInterval);
        }
        Binance.options.log(
        ...
        if (Binance.options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && parseInt(this.endpoint.length, 10) === 60)
                Binance.options.log(
                    "Futures account data WebSocket reconnecting..."
                );
            else
                Binance.options.log(
    ...
    const futuresSubscribeSingle = function (endpoint, callback, params = {}) {
        ...
        } else if (Binance.options.localAddress) {
        ...
        if (Binance.options.verbose)
            Binance.options.log(
                "futuresSubscribeSingle: Subscribed to " + endpoint
            );
        ws.reconnect = Binance.options.reconnect;
        ws.endpoint = endpoint;
        ws.isAlive = false;
    ...
    const futuresSubscribe = function (streams, callback, params = {}) {
        ...
        if (socksproxy !== false) {
            ...
            ws = new WebSocket(
                (Binance.options.test ? fstreamTest : fstream) + queryParams,
                { agent }
            );
        } else if (httpsproxy !== false) {
            if (Binance.options.verbose)
        ...
        ws.on("message", (data) => {
            try {
                ...
                Binance.options.log(
                    `futuresSubscribe: Parse error: ${error.message}`
                );
            }
        });
        return ws;
    ...
    const futuresKlineConcat = (symbol, interval) => {
        ...
        if (
            typeof Binance.futuresRealtime[symbol][interval].time ===
            "undefined"
        )
            return output;
    ...
    const futuresKlineHandler = (symbol, kline, firstTime = 0) => {
        ...
        if (first_updated)
            delete Binance.futuresTicks[symbol][interval][first_updated];
    ...
    const fUserDataAccountUpdateConvertData = (data) => {
        ...
        let updateConverter = (updateData) => {
            let { m: eventReasonType, B: balances, P: positions } = updateData;
            let positionConverter = (position) => {
                let {
                    s: symbol,
                    pa: positionAmount,
    ...
    /**
     * Converts the futures UserData stream TRADE_LITE data into a friendly object
     * @param {object} data - user data callback data type
     * @return {object} - user friendly data type
     */

    const fUserDataTradeLiteConvertData = (data) => {
        let {
            e: eventType,
            E: eventTime,
        ...
        return {
            ...
            originalQuantity,
            originalPrice,
            isMakerSide, // is this trade maker side
            clientOrderId,
            side,
            lastFilledPrice,
            orderLastFilledQuantity,
    ...
    const deliverySocketHeartbeat = () => {
        ...
        for (let endpointId in Binance.deliverySubscriptions) {
            ...
            if (ws.isAlive) {
                ...
                if (Binance.options.verbose)
                    Binance.options.log(
                        `Terminating zombie delivery WebSocket: ${ws.endpoint}`
                    );
                if (ws.readyState === WebSocket.OPEN) ws.terminate();
            }
        }
    ...
    const handleDeliverySocketClose = function (reconnect, code, reason) {
        ...
        if (
            Binance.deliverySubscriptions &&
            Object.keys(Binance.deliverySubscriptions).length === 0
        ) {
            clearInterval(Binance.socketHeartbeatInterval);
        }
        Binance.options.log(
        ...
        if (Binance.options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && parseInt(this.endpoint.length, 10) === 60)
                Binance.options.log(
                    "Delivery account data WebSocket reconnecting..."
                );
            else
                Binance.options.log(
                    "Delivery WebSocket reconnecting: " + this.endpoint + "..."
                );
            try {
                reconnect();
            } catch (error) {
                Binance.options.log(
                    "Delivery WebSocket reconnect error: " + error.message
                );
            }
        }
    };
    ...
    const deliverySubscribeSingle = function (endpoint, callback, params = {}) {
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log(
                    `deliverySubscribeSingle: using socks proxy server: ${socksproxy}`
                );
            let agent = new SocksProxyAgent({
                protocol: parseProxy(socksproxy)[0],
                host: parseProxy(socksproxy)[1],
            ...
            ws = new WebSocket(
                (Binance.options.test ? dstreamSingleTest : dstreamSingle) +
                    endpoint,
                { agent }
            );
        } else if (httpsproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log(
                    `deliverySubscribeSingle: using proxy server: ${agent}`
                );
            ws = new WebSocket(
                (Binance.options.test ? dstreamSingleTest : dstreamSingle) +
                    endpoint,
        ...
        if (Binance.options.verbose)
            Binance.options.log(
                "deliverySubscribeSingle: Subscribed to " + endpoint
            );
        ws.reconnect = Binance.options.reconnect;
        ws.endpoint = endpoint;
        ws.isAlive = false;
    ...
    const deliverySubscribe = function (streams, callback, params = {}) {
        if (typeof streams === "string")
            return deliverySubscribeSingle(streams, callback, params);
        if (typeof params === "boolean") params = { reconnect: params };
        if (!params.reconnect) params.reconnect = false;
        if (!params.openCallback) params.openCallback = false;
        if (!params.id) params.id = false;
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log(
                    `deliverySubscribe: using socks proxy server ${socksproxy}`
                );
            let agent = new SocksProxyAgent({
                protocol: parseProxy(socksproxy)[0],
                host: parseProxy(socksproxy)[1],
            ...
            ws = new WebSocket(
                (Binance.options.test ? dstreamTest : dstream) + queryParams,
                { agent }
            );
        } else if (httpsproxy !== false) {
            if (Binance.options.verbose)
                Binance.options.log(
                    `deliverySubscribe: using proxy server ${httpsproxy}`
                );
            let config = url.parse(httpsproxy);
            let agent = new HttpsProxyAgent(config);
            ws = new WebSocket(
        ...
        if (Binance.options.verbose) {
            Binance.options.log(
                `deliverySubscribe: Subscribed to [${ws.endpoint}] ${queryParams}`
            );
        }
        ws.on("open", handleDeliverySocketOpen.bind(ws, params.openCallback));
        ws.on("pong", handleDeliverySocketHeartbeat);
        ...
        ws.on("message", (data) => {
            try {
                ...
                Binance.options.log(
                    `deliverySubscribe: Parse error: ${error.message}`
                );
            }
        });
        return ws;
    ...
    const deliveryKlineConcat = (symbol, interval) => {
        ...
        if (
            typeof Binance.deliveryRealtime[symbol][interval].time ===
            "undefined"
        )
            return output;
    ...
    const deliveryKlineHandler = (symbol, kline, firstTime = 0) => {
        ...
        if (first_updated)
            delete Binance.deliveryTicks[symbol][interval][first_updated];
    ...
    /**
     * Converts the portfolio margin UserData stream ACCOUNT_UPDATE data into a friendly object
     * @param {object} data - user data callback data type
     * @return {object} - user friendly data type
     */
    const pmUserDataAccountUpdateConvertData = (data) => {
        let {
            e: eventType,
            fs: businessUnit,
            E: eventTime,
    ...
    /**
     * Converts the portfolio margin UserData stream ORDER_TRADE_UPDATE data into a friendly object
     * @param {object} data - user data callback data type
     * @return {object} - user friendly data type
     */
    const pmUserDataOrderUpdateConvertData = (data) => {
        let {
            e: eventType,
            E: eventTime,
            T: transaction, // transaction time
            fs: businessUnit, // 事件业务线：'UM'代表U本位合约，'CM'代表币本位合约
            o: order,
        } = data;
        ...
        let orderConverter = (order) => {
            let {
                ...
                ps: positionSide,
                rp: realizedProfit,
                st: activationPrice, // only pushed with TRAILING_STOP_MARKET order
            } = order;
            return {
                symbol,
                clientOrderId,
    ...
    /**
     * Converts the portofilio margin UserData stream ACCOUNT_CONFIG_UPDATE into a friendly object
     * @param {object} data - user config callback data type
     * @return {object} - user friendly data type
     */
    const pmUserConfigDataAccountUpdateConvertData = (data) => {
        return {
            eventType: data.e,
            businessUnit: data.fs,
            eventTime: data.E,
            transactionTime: data.T,
            ac: {
                symbol: data.ac.s,
    ...
    const userDataHandler = (data) => {
        ...
        if (type === "outboundAccountInfo") {
            ...
            if (Binance.options.execution_callback)
                Binance.options.execution_callback(data);
        } else if (type === "listStatus") {
            if (Binance.options.list_status_callback)
                Binance.options.list_status_callback(data);
    ...
    const userFutureDataHandler = (data) => {
        ...
        if (type === "MARGIN_CALL") {
            ...
            if (Binance.options.future_trade_lite_callback) {
                Binance.options.future_trade_lite_callback(
                    fUserDataTradeLiteConvertData(data)
                );
            }
        } else {
     ...
     * @return {undefined}
     */
    const userPmDataHandler = (data) => {
        let type = data.e;
        if (type === "ACCOUNT_UPDATE") {
            if (Binance.options.pm_account_update_callback) {
                Binance.options.pm_account_update_callback(
                    pmUserDataAccountUpdateConvertData(data)
                );
            }
        } else if (type === "ORDER_TRADE_UPDATE") {
            if (Binance.options.pm_order_update_callback) {
                Binance.options.pm_order_update_callback(
                    pmUserDataOrderUpdateConvertData(data)
                );
            }
        } else if (type === "ACCOUNT_CONFIG_UPDATE") {
            if (Binance.options.pm_account_config_update_callback) {
                Binance.options.pm_account_config_update_callback(
                    pmUserConfigDataAccountUpdateConvertData(data)
                );
            }
        } else {
            Binance.options.log("Unexpected userFutureData: " + type);
    ...
    const klineHandler = (symbol, kline, firstTime = 0) => {
        ...
        if (!isFinal) {
            if (
                typeof Binance.ohlcLatest[symbol][interval].time !== "undefined"
            ) {
        ...
        let updateDepthCache = () => {
            ...
            for (obj of depth.b) {
                ...
                if (obj[1] == 0) {
                    ...
                    Binance.depthCache[symbol].bids[obj[0]] = parseFloat(
                        obj[1]
                    );
                }
            }
            for (obj of depth.a) {
                ...
                if (obj[1] == 0) {
                    ...
                    Binance.depthCache[symbol].asks[obj[0]] = parseFloat(
                        obj[1]
                    );
                }
            }
        ...
        if (context.lastEventUpdateId) {
            ...
            if (depth.U <= expectedUpdateId) {
                ...
                let msg =
                    "depthHandler: [" +
                    symbol +
                    "] The depth cache is out of sync.";
                msg +=
    ...
    const getDepthCache = (symbol) => {
        if (typeof Binance.depthCache[symbol] === "undefined")
            return { bids: {}, asks: {} };
    ...
    /**
     * Used to subscribe to a combined portfolio margin websocket endpoint
     * @param {string} streams - streams to connect to
     * @param {function} callback - the function to call when information is received
     * @param {object} params - Optional reconnect {boolean} (whether to reconnect on disconnect), openCallback {function}, id {string}
     * @return {WebSocket} - websocket reference
     */
    const pmSubscribe = function (streams, callback, params = {}) {
        if (typeof streams === "string")
            return pmSubscribeSingle(streams, callback, params);
        if (typeof params === "boolean") params = { reconnect: params };
        if (!params.reconnect) params.reconnect = false;
        if (!params.openCallback) params.openCallback = false;
        if (!params.id) params.id = false;
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log(
                    `pmSubscribe: using socks proxy server ${socksproxy}`
                );
            let agent = new SocksProxyAgent({
                protocol: parseProxy(socksproxy)[0],
                host: parseProxy(socksproxy)[1],
            ...
            ws = new WebSocket(pstream + queryParams, { agent });
        } else if (httpsproxy !== false) {
            if (Binance.options.verbose)
                Binance.options.log(
                    `pmSubscribe: using proxy server ${httpsproxy}`
                );
            let config = url.parse(httpsproxy);
            let agent = new HttpsProxyAgent(config);
            ws = new WebSocket(pstream + queryParams, { agent });
        } else {
            ws = new WebSocket(pstream + queryParams);
        }
        ...
        ws.on("open", handlePmSocketOpen.bind(ws, params.openCallback));
        ws.on("pong", handlePmSocketHeartbeat);
        ws.on("error", handlePmSocketError);
        ws.on("close", handlePmSocketClose.bind(ws, params.reconnect));
        ws.on("message", (data) => {
            try {
                callback(JSON.parse(data).data);
            } catch (error) {
    ...
    const handlePmSocketOpen = function (openCallback) {
        this.isAlive = true;
        if (Object.keys(Binance.pmSubscriptions).length === 0) {
            Binance.socketHeartbeatInterval = setInterval(
                futuresSocketHeartbeat,
                30000
            );
        }
        Binance.pmSubscriptions[this.endpoint] = this;
        if (typeof openCallback === "function") openCallback(this.endpoint);
    };

    /**
     * Called when pm websocket is closed, subscriptions are de-registered for later reference
     * @param {boolean} reconnect - true or false to reconnect the socket
     * @param {string} code - code associated with the socket
     * @param {string} reason - string with the response
     * @return {undefined}
     */
    const handlePmSocketClose = function (reconnect, code, reason) {
        delete Binance.pmSubscriptions[this.endpoint];
        if (
            Binance.pmSubscriptions &&
            Object.keys(Binance.pmSubscriptions).length === 0
        ) {
            clearInterval(Binance.socketHeartbeatInterval);
        }
        Binance.options.log(
        ...
        if (Binance.options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && parseInt(this.endpoint.length, 10) === 60)
                Binance.options.log(
                    "Pm account data WebSocket reconnecting..."
                );
            else
                Binance.options.log(
                    "Pm WebSocket reconnecting: " + this.endpoint + "..."
                );
            try {
                reconnect();
            } catch (error) {
                Binance.options.log(
                    "Pm WebSocket reconnect error: " + error.message
                );
            }
        }
    };
     ...
     * @return {undefined}
     */
    const handlePmSocketError = function (error) {
        Binance.options.log(
            "Pm WebSocket error: " +
                this.endpoint +
                (error.code ? " (" + error.code + ")" : "") +
     ...
     * @return {undefined}
     */
    const handlePmSocketHeartbeat = function () {
        this.isAlive = true;
    };
     ...
     * @return {WebSocket} - websocket reference
     */
    const pmSubscribeSingle = function (endpoint, callback, params = {}) {
        if (typeof params === "boolean") params = { reconnect: params };
        if (!params.reconnect) params.reconnect = false;
        if (!params.openCallback) params.openCallback = false;
        if (!params.id) params.id = false;
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log(
                    `pmSubscribeSingle: using socks proxy server: ${socksproxy}`
                );
            let agent = new SocksProxyAgent({
                protocol: parseProxy(socksproxy)[0],
                host: parseProxy(socksproxy)[1],
            ...
            ws = new WebSocket(pstreamSingle + endpoint, { agent });
        } else if (httpsproxy !== false) {
            let config = url.parse(httpsproxy);
            let agent = new HttpsProxyAgent(config);
            if (Binance.options.verbose)
                Binance.options.log(
                    `pmSubscribeSingle: using proxy server: ${agent}`
                );
            ws = new WebSocket(pstreamSingle + endpoint, { agent });
        } else {
            ws = new WebSocket(pstreamSingle + endpoint);
        }

        if (Binance.options.verbose)
            Binance.options.log("pmSubscribeSingle: Subscribed to " + endpoint);
        ws.reconnect = Binance.options.reconnect;
        ws.endpoint = endpoint;
        ws.isAlive = false;
        ws.on("open", handlePmSocketOpen.bind(ws, params.openCallback));
    ...
    const orderSocketHeartbeat = () => {
        /* Sockets removed from subscriptions during a manual terminate()
         will no longer be at risk of having functions called on them */
        for (let endpointId in Binance.orderConnections) {
            const ws = Binance.orderConnections[endpointId];
            if (ws.isAlive) {
                ws.isAlive = false;
                if (ws.readyState === WebSocket.OPEN) ws.ping(noop);
            } else {
    ...
    const handleOrderSocketOpen = function (openCallback) {
        this.isAlive = true;
        if (Object.keys(Binance.orderConnections).length === 0) {
            Binance.socketHeartbeatInterval = setInterval(
                orderSocketHeartbeat,
                30000
            );
        }

        Binance.orderConnections[this.endpoint] = this;
        if (typeof openCallback === "function") openCallback(this.endpoint);
    };

    /**
     * Called when order websocket is closed
     * @param {boolean} reconnect - true or false to reconnect the socket
     * @param {string} code - code associated with the socket
     * @param {string} reason - string with the response
     * @return {undefined}
    ...
    const handleOrderSocketClose = function (reconnect, code, reason) {
        ...
        if (Binance.options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && parseInt(this.endpoint.length, 10) === 60)
                Binance.options.log("Order WebSocket reconnecting...");
            else
                Binance.options.log(
                    "Order WebSocket reconnecting: " + this.endpoint + "..."
                );
            try {
                reconnect();
            } catch (error) {
                Binance.options.log(
                    "Order WebSocket reconnect error: " + error.message
                );
            }
        }
    };
     ...
     * @return {undefined}
     */
    const handleOrderSocketError = function (error) {
        Binance.options.log(
     ...
     * @return {undefined}
     */
    const handleOrderSocketHeartbeat = function (data) {
        this.isAlive = true;
    };

    const initOrderWs = function (callback, params = {}) {
        if (typeof params === "boolean") params = { reconnect: params };
        if (!params.reconnect) params.reconnect = false;
        if (!params.openCallback) params.openCallback = false;
        if (!params.id) params.id = false;
        if (params.wsUrl) {
            orderWs = params.wsUrl;
            delete params.wsUrl;
        }
        let httpsproxy = process.env.https_proxy || false;
        let socksproxy = process.env.socks_proxy || false;
        ...
        if (socksproxy !== false) {
            ...
            if (Binance.options.verbose)
                Binance.options.log(
                    `initOrderWs: using socks proxy server: ${socksproxy}`
                );
            let agent = new SocksProxyAgent({
                protocol: parseProxy(socksproxy)[0],
                host: parseProxy(socksproxy)[1],
            ...
            ws = new WebSocket(orderWs, { agent });
        } else if (httpsproxy !== false) {
            let config = url.parse(httpsproxy);
            let agent = new HttpsProxyAgent(config);
            if (Binance.options.verbose)
                Binance.options.log(
                    `initOrderWs: using proxy server: ${agent}`
                );
            ws = new WebSocket(orderWs, { agent });
        } else {
            if (Binance.options.localAddress) {
                ws = new WebSocket(orderWs, {
                    localAddress: Binance.options.localAddress,
                });
            } else {
                ws = new WebSocket(orderWs);
    ...
    const orderLogon = function (id) {
        requireApiSecret("signedRequest");
        let data = {
            apiKey: Binance.options.APIKEY,
            timestamp: new Date().getTime() + Binance.info.timeOffset,
        };
        let query = makeQueryString(data);
        const privateKey = createPrivateKey({
            key: Binance.options.APISECRET,
            format: "pem",
            type: "pkcs8",
        });

        let signature = crypto
            .sign(null, Buffer.from(query), privateKey)
            .toString("base64");
    ...
    return {
        ...
        roundTicks: function (price, tickSize) {
            ...
            const precision =
                formatter.format(tickSize).split(".")[1].length || 0;
         ...
         * @return {promise or undefined} - omitting the callback returns a promise
         */
        fundingBalance: function (callback = false) {
            if (!callback) {
                return new Promise((resolve, reject) => {
                    callback = (error, response) => {
                        if (error) {
        ...
        futuresTime: async (params = {}) => {
            return promiseRequest("v1/time", params, { base: fapi }).then(
                (r) => r.serverTime
            );
        },
        ...
        futuresMarkPrice: async (symbol = false) => {
            return promiseRequest("v1/premiumIndex", symbol ? { symbol } : {}, {
                base: fapi,
            });
        },
        ...
        futuresOrderRateLimit: async (params = {}) => {
            return promiseRequest("v1/rateLimit/order", params, {
                base: fapi,
                type: "SIGNED",
            });
        },
        ...
        futuresCountdownCancelAll: async (
            symbol,
            countdownTime = 0,
            params = {}
        ) => {
        ...
        futuresConState: async (params = {}) => {
            return promiseRequest("v1/clientConState", params, {
                base: fapi,
                type: "SIGNED",
            });
        },
        ...
        futuresChangePositionSideDual: async (
            dualSidePosition,
            params = {}
        ) => {
        ...
        */
        ...
        deliveryTime: async (params = {}) => {
            return promiseRequest("v1/time", params, { base: dapi }).then(
                (r) => r.serverTime
            );
        },
        ...
        deliveryPrices: async (params = {}) => {
            ...
            return data.reduce(
                (out, i) => ((out[i.symbol] = i.price), out),
                {}
            );
        },
        ...
        deliveryCountdownCancelAll: async (
            symbol,
            countdownTime = 0,
            params = {}
        ) => {
        ...
        deliveryChangePositionSideDual: async (
            dualSidePosition,
            params = {}
        ) => {
            params.dualSidePosition = dualSidePosition;
        ...
        mgCancel: function (
            ...
            signedRequest(
                ...
                { symbol: symbol, orderId: orderid, isIsolated },
                function (error, data) {
                    if (callback)
                        return callback.call(this, error, data, symbol);
                },
        ...
        futuresTerminate: function (endpoint) {
            if (Binance.options.verbose)
                Binance.options.log("Futures WebSocket terminating:", endpoint);
        ...
        futuresMarkPriceStream: function fMarkPriceStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    fMarkPriceStream(symbol, callback, speed);
            };
        ...
        futuresLiquidationStream: function fLiquidationStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    fLiquidationStream(symbol, callback);
            };
            const endpoint = symbol
        ...
        futuresMiniTickerStream: function fMiniTickerStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    fMiniTickerStream(symbol, callback);
            };
            const endpoint = symbol
        ...
        futuresBookTickerStream: function fBookTickerStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    fBookTickerStream(symbol, callback);
            };
            const endpoint = symbol
         ...
         * @return {string} the websocket endpoint
         */
        futuresChart: async function futuresChart(
            symbols,
            interval,
            callback,
            limit = 500
            ...
            let futuresChartInit = (symbol) => {
                if (typeof Binance.futuresMeta[symbol] === "undefined")
                    Binance.futuresMeta[symbol] = {};
                if (
                    typeof Binance.futuresMeta[symbol][interval] === "undefined"
                )
                    Binance.futuresMeta[symbol][interval] = {};
                if (typeof Binance.futuresTicks[symbol] === "undefined")
                    Binance.futuresTicks[symbol] = {};
                if (
                    typeof Binance.futuresTicks[symbol][interval] ===
                    "undefined"
                )
                    Binance.futuresTicks[symbol][interval] = {};
                ...
                if (
                    typeof Binance.futuresRealtime[symbol][interval] ===
                    "undefined"
                )
                    Binance.futuresRealtime[symbol][interval] = {};
                ...
                if (
                    typeof Binance.futuresKlineQueue[symbol][interval] ===
                    "undefined"
                )
            ...
            let getFuturesKlineSnapshot = async (symbol, limit = 500) => {
                ...
                if (
                    ...
                    for (let kline of Binance.futuresKlineQueue[symbol][
                        interval
                    ])
                        futuresKlineHandler(
                            symbol,
                            kline,
                            Binance.futuresMeta[symbol][interval].timestamp
                        );
            ...
            if (Array.isArray(symbols)) {
                ...
                symbols.forEach((element) =>
                    getFuturesKlineSnapshot(element, limit)
                );
            } else {
                let symbol = symbols;
        ...
        deliveryTerminate: function (endpoint) {
            if (Binance.options.verbose)
                Binance.options.log(
                    "Delivery WebSocket terminating:",
                    endpoint
                );
        ...
        deliveryAggTradeStream: function deliveryAggTradeStream(
            ...
            if (Array.isArray(symbols)) {
                if (!isArrayUnique(symbols))
                    throw Error(
                        'deliveryAggTradeStream: "symbols" cannot contain duplicate elements.'
                    );
                let streams = symbols.map(
                    (symbol) => symbol.toLowerCase() + "@aggTrade"
                );
        ...
        deliveryMarkPriceStream: function dMarkPriceStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    dMarkPriceStream(symbol, callback);
            };
            const endpoint = symbol
                ? `${symbol.toLowerCase()}@markPrice`
                : "!markPrice@arr";
        ...
        deliveryLiquidationStream: function dLiquidationStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    dLiquidationStream(symbol, callback);
            };
            const endpoint = symbol
                ? `${symbol.toLowerCase()}@forceOrder`
                : "!forceOrder@arr";
        ...
        deliveryMiniTickerStream: function dMiniTickerStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    dMiniTickerStream(symbol, callback);
            };
            const endpoint = symbol
                ? `${symbol.toLowerCase()}@miniTicker`
                : "!miniTicker@arr";
        ...
        deliveryBookTickerStream: function dBookTickerStream(
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    dBookTickerStream(symbol, callback);
            };
            const endpoint = symbol
                ? `${symbol.toLowerCase()}@bookTicker`
                : "!bookTicker";
         ...
         * @return {string} the websocket endpoint
         */
        deliveryChart: async function deliveryChart(
            symbols,
            interval,
            callback,
            limit = 500
            ...
            let reconnect = () => {
                if (Binance.options.reconnect)
                    deliveryChart(symbols, interval, callback, limit);
            };

            let deliveryChartInit = (symbol) => {
                if (typeof Binance.deliveryMeta[symbol] === "undefined")
                    Binance.deliveryMeta[symbol] = {};
                if (
                    typeof Binance.deliveryMeta[symbol][interval] ===
                    "undefined"
                )
                ...
                if (
                    typeof Binance.deliveryTicks[symbol][interval] ===
                    "undefined"
                )
                    Binance.deliveryTicks[symbol][interval] = {};
                ...
                if (
                    typeof Binance.deliveryRealtime[symbol][interval] ===
                    "undefined"
                )
                    Binance.deliveryRealtime[symbol][interval] = {};
                ...
                if (
                    typeof Binance.deliveryKlineQueue[symbol][interval] ===
                    "undefined"
                )
            ...
            let handleDeliveryKlineStream = (kline) => {
                ...
                if (!Binance.deliveryMeta[symbol][interval].timestamp) {
                    if (
                        ...
                        Binance.deliveryKlineQueue[symbol][interval].push(
                            kline
                        );
                    }
                } else {
            ...
            let getDeliveryKlineSnapshot = async (symbol, limit = 500) => {
                ...
                if (
                    ...
                    for (let kline of Binance.deliveryKlineQueue[symbol][
                        interval
                    ])
                        deliveryKlineHandler(
                            symbol,
                            kline,
                            Binance.deliveryMeta[symbol][interval].timestamp
                        );
            ...
            if (Array.isArray(symbols)) {
                if (!isArrayUnique(symbols))
                    throw Error(
                        'deliveryChart: "symbols" array cannot contain duplicate elements.'
                    );
                symbols.forEach(deliveryChartInit);
                let streams = symbols.map(
                    (symbol) => `${symbol.toLowerCase()}@kline_${interval}`
                );
                subscription = deliverySubscribe(
                    streams,
                    handleDeliveryKlineStream,
                    reconnect
                );
                symbols.forEach((element) =>
                    getDeliveryKlineSnapshot(element, limit)
                );
            } else {
                let symbol = symbols;
        ...
        deliveryCandlesticks: function deliveryCandlesticks(
            ...
            if (Array.isArray(symbols)) {
                if (!isArrayUnique(symbols))
                    throw Error(
                        'deliveryCandlesticks: "symbols" array cannot contain duplicate elements.'
                    );
                let streams = symbols.map(
                    (symbol) => symbol.toLowerCase() + "@kline_" + interval
                );
        ...
        pmGetUmPositions: async (params = {}) => {
            return promiseRequest("v1/um/positionRisk", params, {
                base: papi,
                type: "SIGNED",
            });
        },

        pmGetUmOpenOrders: async (params = {}) => {
            return promiseRequest("v1/um/openOrders", params, {
                base: papi,
                type: "SIGNED",
            });
        },

        pmGetUmCommissionRate: async (symbol, params = {}) => {
            if (symbol) params.symbol = symbol;
            return promiseRequest("v1/um/commissionRate", params, {
                base: papi,
                type: "SIGNED",
            });
        },

        pmPlaceUmOrder: pmUmOrder,

        pmCancelUmOrder: async (symbol, params = {}) => {
            // Either orderId or origClientOrderId must be sent
            params.symbol = symbol;
            return promiseRequest("v1/um/order", params, {
                base: papi,
        ...
        websockets: {
                ...
                apiRequest(
                    ...
                    function (error, response) {
                        ...
                        Binance.options.execution_callback = execution_callback
                            ? callback
                            : execution_callback; //This change is required to listen for Orders
                        Binance.options.list_status_callback =
                ...
                apiRequest(
                    ...
                    function (error, response) {
                        ...
                        Binance.options.margin_execution_callback =
                            execution_callback;
                        Binance.options.margin_list_status_callback =
                            list_status_callback;
                        const subscription = subscribe(
                            Binance.options.listenMarginKey,
            ...
            /**
             * Portfolio margin account userdata websockets function
             * @param {function} account_update_callback
             * @param {function} order_update_callback
             * @param {Function} subscribed_callback - subscription callback
             */
            userPmData: function userPmData(
                account_update_callback = undefined,
                order_update_callback = undefined,
                subscribed_callback = undefined,
                account_config_update_callback = undefined
                ...
                apiRequest(
                    ...
                    function (error, response) {
                        ...
                        Binance.options.pm_order_update_callback =
                            order_update_callback;
                        const subscription = pmSubscribe(
                            Binance.options.listenFutureKey,
                            userPmDataHandler,
                            { reconnect }
                        );
            ...
            wsFuturesPlaceOrder: function wsFuturesPlaceOrder(request) {
                const wsKey = "futures";
                // if connected, send subscription request
                if (isConnectionState(wsKey, "CONNECTED")) {
                    const wsMessage = JSON.stringify(request);
                    //console.log(wsKey, wsMessage);
                    return tryWsSend(wsKey, wsMessage);
                }
                // start connection process if it hasn't yet begun. Topics are automatically subscribed to on-connect
                if (
                    !isConnectionState(wsKey, "CONNECTING") &&
                    !isConnectionState(wsKey, "RECONNECTING")
                ) {
                    return wsConnect(wsKey);
                }
            },
            /**
            ...
            terminate: function (endpoint) {
                if (Binance.options.verbose)
                    Binance.options.log("WebSocket terminating:", endpoint);
            ...
            depth: function depth(symbols, callback) {
                ...
                if (Array.isArray(symbols)) {
                    if (!isArrayUnique(symbols))
                        throw Error(
                            'depth: "symbols" cannot contain duplicate elements.'
                        );
            ...
            depthCache: function depthCacheFunction(
                ...
                let symbolDepthInit = (symbol) => {
                    if (
                        typeof Binance.depthCacheContext[symbol] === "undefined"
                    )
                ...
                if (Array.isArray(symbols)) {
                    if (!isArrayUnique(symbols))
                        throw Error(
                            'depthCache: "symbols" cannot contain duplicate elements.'
                        );
            ...
            aggTrades: function trades(symbols, callback) {
                ...
                if (Array.isArray(symbols)) {
                    if (!isArrayUnique(symbols))
                        throw Error(
                            'trades: "symbols" cannot contain duplicate elements.'
                        );
                    let streams = symbols.map(function (symbol) {
            ...
            chart: function chart(symbols, interval, callback, limit = 500) {
                let reconnect = () => {
                    if (Binance.options.reconnect)
                        chart(symbols, interval, callback, limit);
                };

                let symbolChartInit = (symbol) => {
                    if (typeof Binance.info[symbol] === "undefined")
                        Binance.info[symbol] = {};
                    if (typeof Binance.info[symbol][interval] === "undefined")
                        Binance.info[symbol][interval] = {};
                    if (typeof Binance.ohlc[symbol] === "undefined")
                        Binance.ohlc[symbol] = {};
                    if (typeof Binance.ohlc[symbol][interval] === "undefined")
                        Binance.ohlc[symbol][interval] = {};
                    if (typeof Binance.ohlcLatest[symbol] === "undefined")
                        Binance.ohlcLatest[symbol] = {};
                    if (
                        typeof Binance.ohlcLatest[symbol][interval] ===
                        "undefined"
                    )
                        Binance.ohlcLatest[symbol][interval] = {};
                    if (typeof Binance.klineQueue[symbol] === "undefined")
                        Binance.klineQueue[symbol] = {};
                    if (
                        typeof Binance.klineQueue[symbol][interval] ===
                        "undefined"
                    )
                    ...
                    publicRequest(
                        ...
                        function (error, data) {
                            ...
                            if (
                                ...
                                for (let kline of Binance.klineQueue[symbol][
                                    interval
                                ])
                                    klineHandler(
                                        symbol,
                                        kline,
                                        Binance.info[symbol][interval].timestamp
                                    );
                ...
                if (Array.isArray(symbols)) {
                    if (!isArrayUnique(symbols))
                        throw Error(
                            'chart: "symbols" cannot contain duplicate elements.'
                        );
                    ...
                    symbols.forEach((element) =>
                        getSymbolKlineSnapshot(element, limit)
                    );
                } else {
                    let symbol = symbols;
            ...
            bookTickers: function bookTickerStream(
                ...
                let reconnect = () => {
                    if (Binance.options.reconnect)
                        bookTickerStream(symbol, callback);
                };
                const endpoint = symbol
                    ? `${symbol.toLowerCase()}@bookTicker`
                    : "!bookTicker";
            ...
            prevDay: function prevDay(symbols, callback, singleCallback) {
                let reconnect = () => {
                    if (Binance.options.reconnect)
                        prevDay(symbols, callback, singleCallback);
                };
                ...
                if (Array.isArray(symbols)) {
                    if (!isArrayUnique(symbols))
                        throw Error(
                            'prevDay: "symbols" cannot contain duplicate elements.'
                        );
                    let streams = symbols.map(function (symbol) {
            ...
            initOrderWs: function (callback, reconnect = false) {
                return initOrderWs(callback, reconnect);
            },

            orderLogon: function (id) {
                return orderLogon(id);
            },

            wsPlaceOrder: function (id, symbol, side, quantity, price, params) {
                return wsPlaceOrder(id, symbol, side, quantity, price, params);
            },
