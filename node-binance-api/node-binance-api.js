...
let api = function Binance( options = {} ) {
    ...
    const pmUmOrder = async ( side, symbol, quantity, price = false, params = {} ) => {
        ...
        return promiseRequest( 'v1/um/order', params, { base:papi, type:'TRADE', method:'POST' } );
    };

    const pmCmOrder = async ( side, symbol, quantity, price = false, params = {} ) => {
        params.symbol = symbol;
        params.side = side;
        if ( quantity ) params.quantity = quantity;
        // if in the binance futures setting Hedged mode is active, positionSide parameter is mandatory
        ...
        return promiseRequest( 'v1/cm/order', params, { base:papi, type:'TRADE', method:'POST' } );
    };
    ...
    const userFutureDataHandler = data => {
        ...
        if ( type === 'MARGIN_CALL' ) {
            ...
            if (Binance.options.future_trade_lite_callback) {
                Binance.options.future_trade_lite_callback( fUserDataTradeLiteConvertData( data ) );
            }
        } else {
    ...
    const userPmDataHandler = data => {
        ...
        if ( type === 'ACCOUNT_UPDATE' ) {
            if ( Binance.options.pm_account_update_callback ) {
                Binance.options.pm_account_update_callback( pmUserDataAccountUpdateConvertData( data ) );
            }
        } else if ( type === 'ORDER_TRADE_UPDATE' ) {
            if ( Binance.options.pm_order_update_callback ) {
                Binance.options.pm_order_update_callback( pmUserDataOrderUpdateConvertData( data ) );
            }
        } else if ( type === 'ACCOUNT_CONFIG_UPDATE' ) {
            if ( Binance.options.pm_account_config_update_callback ) {
                Binance.options.pm_account_config_update_callback( pmUserConfigDataAccountUpdateConvertData( data ) );
            }
        } else {
            Binance.options.log( 'Unexpected userFutureData: ' + type );
        }
   ...
   */
    ...
    const pmSubscribe = function ( streams, callback, params = {} ) {
        if ( typeof streams === 'string' ) return pmSubscribeSingle( streams, callback, params );
        if ( typeof params === 'boolean' ) params = { reconnect: params };
        if ( !params.reconnect ) params.reconnect = false;
        if ( !params.openCallback ) params.openCallback = false;
        if ( !params.id ) params.id = false;
        ...
        if ( socksproxy !== false ) {
            socksproxy = proxyReplacewithIp( socksproxy );
            if ( Binance.options.verbose ) Binance.options.log( `pmSubscribe: using socks proxy server ${ socksproxy }` );
            let agent = new SocksProxyAgent( {
                protocol: parseProxy( socksproxy )[0],
                host: parseProxy( socksproxy )[1],
                port: parseProxy( socksproxy )[2]
            ...
            if ( Binance.options.verbose ) Binance.options.log( `pmSubscribe: using proxy server ${ httpsproxy }` );
            let config = url.parse( httpsproxy );
            let agent = new HttpsProxyAgent( config );
            ws = new WebSocket( pstream + queryParams, { agent } );
        } else {
    ...
    const handlePmSocketClose = function ( reconnect, code, reason ) {
        ...
        if ( Binance.pmSubscriptions && Object.keys( Binance.pmSubscriptions ).length === 0 ) {
            clearInterval( Binance.socketHeartbeatInterval );
        }
        Binance.options.log( 'Pm WebSocket closed: ' + this.endpoint +
          ( code ? ' (' + code + ')' : '' ) +
          ( reason ? ' ' + reason : '' ) );
        if ( Binance.options.reconnect && this.reconnect && reconnect ) {
            if ( this.endpoint && parseInt( this.endpoint.length, 10 ) === 60 ) Binance.options.log( 'Pm account data WebSocket reconnecting...' );
            else Binance.options.log( 'Pm WebSocket reconnecting: ' + this.endpoint + '...' );
            try {
                reconnect();
            } catch ( error ) {
                Binance.options.log( 'Pm WebSocket reconnect error: ' + error.message );
            }
        }
    };
    ...
    const handlePmSocketError = function ( error ) {
        Binance.options.log( 'Pm WebSocket error: ' + this.endpoint +
          ( error.code ? ' (' + error.code + ')' : '' ) +
          ( error.message ? ' ' + error.message : '' ) );
    };
    ...
    const pmSubscribeSingle = function ( endpoint, callback, params = {} ) {
        ...
        if ( socksproxy !== false ) {
            socksproxy = proxyReplacewithIp( socksproxy );
            if ( Binance.options.verbose ) Binance.options.log( `pmSubscribeSingle: using socks proxy server: ${ socksproxy }` );
            let agent = new SocksProxyAgent( {
                protocol: parseProxy( socksproxy )[0],
                host: parseProxy( socksproxy )[1],
                port: parseProxy( socksproxy )[2]
            ...
            let config = url.parse( httpsproxy );
            let agent = new HttpsProxyAgent( config );
            if ( Binance.options.verbose ) Binance.options.log( `pmSubscribeSingle: using proxy server: ${ agent }` );
            ws = new WebSocket( pstreamSingle + endpoint, { agent } );
        } else {
            ws = new WebSocket( pstreamSingle + endpoint );
        }

        if ( Binance.options.verbose ) Binance.options.log( 'pmSubscribeSingle: Subscribed to ' + endpoint );
        ws.reconnect = Binance.options.reconnect;
        ws.endpoint = endpoint;
        ws.isAlive = false;
        ws.on( 'open', handlePmSocketOpen.bind( ws, params.openCallback ) );
    ...
    const handleOrderSocketOpen = function ( openCallback ) {
        ...
        if ( Object.keys( Binance.orderConnections ).length === 0 ) {
            Binance.socketHeartbeatInterval = setInterval( orderSocketHeartbeat, 30000 );
        }
    ...
    const handleOrderSocketClose = function ( reconnect, code, reason ) {
        ...
        Binance.options.log( 'Order WebSocket closed: ' + this.endpoint +
          ( code ? ' (' + code + ')' : '' ) +
          ( reason ? ' ' + reason : '' ) );
        if ( Binance.options.reconnect && this.reconnect && reconnect ) {
            if ( this.endpoint && parseInt( this.endpoint.length, 10 ) === 60 ) Binance.options.log( 'Order WebSocket reconnecting...' );
            else Binance.options.log( 'Order WebSocket reconnecting: ' + this.endpoint + '...' );
            try {
                reconnect();
            } catch ( error ) {
                Binance.options.log( 'Order WebSocket reconnect error: ' + error.message );
            }
        }
    };
    ...
    const handleOrderSocketError = function ( error ) {
        Binance.options.log( 'Order WebSocket error: ' + this.endpoint +
          ( error.code ? ' (' + error.code + ')' : '' ) +
          ( error.message ? ' ' + error.message : '' ) );
    };
    ...
    const initOrderWs = function (callback, params = {} ) {
        ...
        if ( socksproxy !== false ) {
            socksproxy = proxyReplacewithIp( socksproxy );
            if ( Binance.options.verbose ) Binance.options.log( `initOrderWs: using socks proxy server: ${ socksproxy }` );
            let agent = new SocksProxyAgent( {
                protocol: parseProxy( socksproxy )[0],
                host: parseProxy( socksproxy )[1],
                port: parseProxy( socksproxy )[2]
            ...
            let config = url.parse( httpsproxy );
            let agent = new HttpsProxyAgent( config );
            if ( Binance.options.verbose ) Binance.options.log( `initOrderWs: using proxy server: ${ agent }` );
            ws = new WebSocket( orderWs, { agent } );
        } else {
            if (Binance.options.localAddress) {
                ws = new WebSocket( orderWs, {localAddress: Binance.options.localAddress} );
            } else {
        ...
        if ( Binance.options.verbose ) Binance.options.log( 'initOrderWs: create order ws' );
        ws.reconnect = Binance.options.reconnect;
    ...
    const orderLogon = function (id) {
        ...
        let signature= crypto.sign(null, Buffer.from(query), privateKey).toString('base64');
    ...
    const wsPlaceOrder = function(reqId, symbol, side, quantity, price, params) {
        params.symbol = symbol;
        params.side = side;
        if ( quantity ) params.quantity = quantity;
    ...
    return {
        ...
        fundingBalance: function ( callback = false ) {
            if ( !callback ) {
                return new Promise( ( resolve, reject ) => {
                    ...
                    signedRequest( sapi + 'v1/asset/get-funding-asset', {}, function ( error, data ) {
        ...
        */
        ...
        futuresConState: async ( params = {} ) => {
            return promiseRequest( 'v1/clientConState', params, { base:fapi, type:'SIGNED' } );
        },
        ...
        futuresMultiAssetsMargin: async ( params = {} ) => {
            return promiseRequest( 'v1/multiAssetsMargin', params, { base:fapi, type:'SIGNED' } );
        },

        futuresChangeMultiAssetsMargin: async ( multi, params = {} ) => {
            params.multiAssetsMargin = multi;
            return promiseRequest( 'v1/multiAssetsMargin', params, { base:fapi, type:'SIGNED', method:'POST' } );
        },
     ...
     */
        ...
        enableBNBBurn: function ( spotBNBBurn, interestBNBBurn ) {
            if (!["true", "false"].includes(spotBNBBurn) || !["true", "false"].includes(interestBNBBurn)) {
                console.error(`invalid params spotBNBBurn=${spotBNBBurn} and interestBNBBurn=${interestBNBBurn} should be "true" or "false"`)
                return
            }
            ...
            return promiseRequest( 'v1/bnbBurn', parameters, { base:sapi, type:'SIGNED', method:'POST' } );
        },
        ...
        mgAllAssets: function ( asset = null, callback) {
            ...
            return promiseRequest( 'v1/margin/allAssets', parameters, { base:sapi, type:'SIGNED' } );
        },
         ...
         * @return {undefined}
         */
        mgBorrowRepay: function ( asset, amount, type, callback, isIsolated='FALSE',symbol=null ) {
            let parameters = Object.assign( { asset: asset, amount: amount, type: type } );
            if (isIsolated ==='TRUE' && !symbol) throw new Error('If "isIsolated" = "TRUE", "symbol" must be sent')
            const isolatedObj = isIsolated === 'TRUE'?{
                isIsolated,
                symbol
            }:{}
            return promiseRequest( 'v1/margin/borrow-repay', {...parameters,...isolatedObj}, { base:sapi, type:'SIGNED', method:'POST' } );
        },
        ...
        pmGetAccount: async ( params = {} ) => {
            return promiseRequest( 'v1/account', params, { base: papi, type:'SIGNED' } );
        },

        // portfolio相关的处理
        pmGetBalance: async ( params = {} ) => {
            return promiseRequest( 'v1/balance', params, { base: papi, type:'SIGNED' } );
        },

        pmGetUmPositions: async ( params = {} ) => {
            return promiseRequest( 'v1/um/positionRisk', params, { base: papi, type:'SIGNED' } );
        },

        pmGetUmOpenOrders: async ( params = {} ) => {
            return promiseRequest( 'v1/um/openOrders', params, { base: papi, type:'SIGNED' } );
        },

        pmGetUmCommissionRate: async ( symbol, params = {} ) => {
            if ( symbol ) params.symbol = symbol;
            return promiseRequest( 'v1/um/commissionRate', params, { base: papi, type:'SIGNED' } );
        },

        pmPlaceUmOrder: pmUmOrder,

        pmCancelUmOrder: async ( symbol, params = {} ) => { // Either orderId or origClientOrderId must be sent
            params.symbol = symbol;
            return promiseRequest( 'v1/um/order', params, { base:papi, type:'SIGNED', method:'DELETE' } );
        },

        pmCancelAllUmOrders: async ( symbol, params = {} ) => {
            params.symbol = symbol;
            return promiseRequest( 'v1/um/allOpenOrders', params, { base:papi, type:'SIGNED', method:'DELETE' } );
        },

        pmUmPositionSideDual: async ( params = {} ) => {
            return promiseRequest( 'v1/um/positionSide/dual', params, { base:papi, type:'SIGNED' } );
        },

        pmUmChangePositionSideDual: async ( dualSidePosition, params = {} ) => {
            params.dualSidePosition = dualSidePosition;
            return promiseRequest( 'v1/um/positionSide/dual', params, { base:papi, type:'SIGNED', method:'POST' } );
        },

        pmGetCmPositions: async ( params = {} ) => {
            return promiseRequest( 'v1/cm/positionRisk', params, { base: papi, type:'SIGNED' } );
        },

        pmGetCmOpenOrders: async ( params = {} ) => {
            return promiseRequest( 'v1/cm/openOrders', params, { base: papi, type:'SIGNED' } );
        },

        pmGetCmCommissionRate: async ( symbol, params = {} ) => {
            if ( symbol ) params.symbol = symbol;
            return promiseRequest( 'v1/cm/commissionRate', params, { base: papi, type:'SIGNED' } );
        },

        pmPlaceCmOrder: pmCmOrder,

        pmCancelCmOrder: async ( symbol, params = {} ) => { // Either orderId or origClientOrderId must be sent
            params.symbol = symbol;
            return promiseRequest( 'v1/cm/order', params, { base:papi, type:'SIGNED', method:'DELETE' } );
        },

        pmCancelAllCmOrders: async ( symbol, params = {} ) => {
            params.symbol = symbol;
            return promiseRequest( 'v1/cm/allOpenOrders', params, { base:papi, type:'SIGNED', method:'DELETE' } );
        },

        pmCmPositionSideDual: async ( params = {} ) => {
            return promiseRequest( 'v1/cm/positionSide/dual', params, { base:papi, type:'SIGNED' } );
        },

        pmCmChangePositionSideDual: async ( dualSidePosition, params = {} ) => {
            params.dualSidePosition = dualSidePosition;
            return promiseRequest( 'v1/cm/positionSide/dual', params, { base:papi, type:'SIGNED', method:'POST' } );
        },
        ...
        websockets: {
             ...
             * @param {Function} subscribed_callback - subscription callback
             */
            userFutureData: function userFutureData( margin_call_callback, account_update_callback = undefined, order_update_callback = undefined, trade_lite_callback = undefined, subscribed_callback = undefined, account_config_update_callback = undefined ) {
                ...
                apiRequest( url + 'v1/listenKey', {}, function ( error, response ) {
                    ...
                    Binance.options.future_trade_lite_callback = trade_lite_callback ;
           ...
           */
             ...
             * @param {Function} subscribed_callback - subscription callback
             */
            userPmData: function userPmData(account_update_callback = undefined, order_update_callback = undefined, subscribed_callback = undefined, account_config_update_callback = undefined ) {
                const url = papi;
                let reconnect = () => {
                    if ( Binance.options.reconnect ) userPmData(account_update_callback, order_update_callback, subscribed_callback )
                }
                ...
                apiRequest( url + 'v1/listenKey', {}, function ( error, response ) {
                    ...
                    Binance.options.pm_account_update_callback = account_update_callback;
                    Binance.options.pm_account_config_update_callback = account_config_update_callback;
                    Binance.options.pm_order_update_callback = order_update_callback;
                    const subscription = pmSubscribe( Binance.options.listenFutureKey, userPmDataHandler, { reconnect } );
                    if ( subscribed_callback ) subscribed_callback( subscription.endpoint );
                }, 'POST' );
            },
            ...
            wsFuturesPlaceOrder: function wsFuturesPlaceOrder( request ) {
                ...
                if (!isConnectionState(wsKey, 'CONNECTING') &&
